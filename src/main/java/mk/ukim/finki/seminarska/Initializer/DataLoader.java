...
public class DataLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        if(genreRepository.findAll().isEmpty()) {
            ...
            genres.add(new Genre("Animation", Collections.emptyList()));
            genres.add(new Genre("Biography", Collections.emptyList()));
            genres.add(new Genre("Comedy", Collections.emptyList()));
            genres.add(new Genre("Crime", Collections.emptyList()));
            genres.add(new Genre("Documentary", Collections.emptyList()));
            genres.add(new Genre("Drama", Collections.emptyList()));
            genres.add(new Genre("History", Collections.emptyList()));
        ...
        Comment commentOne = new Comment("Amazing","Loved it!!!", null, 10.0, user);
        Comment commentTwo = new Comment("Great one!","Hatidze was exceptional.", null, 10.0, user);
        Comment commentThree = new Comment("NO","I didn't like this!", null, 2.0, admin);
        Comment commentFour = new Comment("Good","Average movie", null, 5.0, user);
        Comment commentFive = new Comment("Not bad","ok", null, 7.3, admin);
        Comment commentSix = new Comment("G R E A T ","Love it", null, 10.0, user);
        ...
        if(movieRepository.findAll().isEmpty()){
            List<Genre> genresHoneyland = genres.stream().filter(g -> g.getName().equalsIgnoreCase("Documentary")).collect(Collectors.toList());
            List<Person> starsHoneyland = people.stream().filter(p -> p.getName().equalsIgnoreCase("Hatidze Muratova")).collect(Collectors.toList());
            List<Person> directorsHoneyland = people.stream().filter(p -> p.getName().equalsIgnoreCase("Tamara Kotevska")).collect(Collectors.toList());
            List<Person> writersHoneyland = people.stream().filter(p -> p.getName().equalsIgnoreCase("Tamara Kotevska")).collect(Collectors.toList());
            List<Comment> commentsHoneyland = new ArrayList<>();
            commentsHoneyland.add(commentTwo);
            commentsHoneyland.add(commentThree);
            List<String> languagesHoneyland = new ArrayList<>();
            languagesHoneyland.add("Turkish");
            languagesHoneyland.add("Macedonian");
            movies.add(new Movie("Honeyland", 2019, "The last female bee-hunter in Europe must save the bees and return the natural balance in Honeyland, when a family of nomadic beekeepers invade her land and threaten her livelihood.", 8.0, genresHoneyland, directorsHoneyland, starsHoneyland, writersHoneyland, null, "Macedonia", "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcR3yg_5VHrhmIjArOd9SliEOQl6K0xEKGUwYnxFrZPSjifGi8IG", 86, "https://www.youtube.com/embed/B27ORUHlp6E", "https://www.imdb.com/title/tt8991268/", languagesHoneyland));
